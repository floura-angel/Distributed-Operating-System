defmodule Gossip_Protocol do
  ...
  def failure(percentage, actors) do
    if percentage > 0 do
      ...
      num = trunc(percentage / 100 * length(indexed_actors))
      # IO.puts(num)
  ...
  def main(arg1, arg2, arg3) do
    ...
    if numberNodes > 0 do
      ...
      if algo == "gossip" do
        ...
        init_algorithm(nodes_worker, topology, numberNodes, algo)
      else
        if algo == "push-sum" do
  ...
  def init_algorithm(nodes_worker, topology, numberNodes, algo) do
    ...
    if algo == "gossip" do
      ...
      gossip(nodes_worker, members, numberNodes)
    else
      # call push-sum algorithm
      push_sum(nodes_worker, members, numberNodes)
    end
  ...
  #  ---------------------   Start Push Sum   ---------------------
  def push_sum(nodes_worker, members, numberNodes) do
    # for  {list_members, y}  <-  members  do
    for {list_members, _} <- members do
